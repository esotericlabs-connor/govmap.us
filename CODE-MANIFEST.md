...
- **ZIP crosswalk auto-discovers its source file.** `zip_crosswalk.py` now lists the Census directory `rel2020/zcta520/` and picks the newest `tab20_zcta520_cd*_natl.txt` (adapts to cd118/cd119/…) instead of a hard-coded filename that 404'd. If no ZCTA↔CD file is present it fails soft and **logs the directory's actual `tab20_*` filenames**, so one deploy reveals the real layout if it differs. `/api/lookup` returns empty until this succeeds (the ZIP lookup on `/congress` shows "couldn't match"). ZCTA≈ZIP is the intended lookup approximation (avoids the auth-gated HUD crosswalk).
