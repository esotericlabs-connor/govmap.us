...
- **FEC finance uses the bulk endpoint.** `fec_finance` pulls `GET /candidates/totals` once per chamber for `FEC_CYCLE` (a handful of paginated calls, ~20 total) and indexes by `candidate_id`, instead of one `/candidate/{id}/totals` call per member (~537). The per-member version exhausted api.data.gov's ~1000 req/hr budget on every redeploy and only ~30% of members loaded; the bulk version fits easily. It's idempotent (upsert on `(bioguide, cycle)`), so a re-run only fills gaps and never regresses existing rows. Note the hourly budget still resets on the hour — if a run 429s because the hour is already spent, wait for the reset.
