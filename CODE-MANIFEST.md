...
- **Deploy runs CORE sources only.** `python -m app.pipelines.refresh` (the deploy step) now refreshes just `members`, `bills`, `votes` — fast and well-behaved. The heavy / rate-limited / slow-cadence sources (`sponsored_bills`, `finance`, `zip_districts`) are in `EXTRA_REFRESHERS`, kept off the deploy hot path: they run on their own scheduler cadence (see `app/scheduler.py`) and on demand via `python -m app.pipelines.refresh <name>` (or `all`). This is because `fec_finance` makes ~537 OpenFEC calls and api.data.gov caps at ~1000 req/hr per key — running it on every redeploy exhausted the budget and only ~30% of members loaded. `fec_finance` now also **paces** calls (~2/sec) and uses a 45s timeout; it's idempotent, so re-running fills any gaps. A fresh environment should run `refresh all` once after the first deploy.
- **ZIP crosswalk URL is wrong (confirmed 404).** `zip_crosswalk.py`'s guessed Census paths (`rel2020/zcta520/tab20_zcta520_cd119/118_natl.txt`) both 404 on the VM — a ZCTA↔CD 2020 national relationship file may not live there. It **fails soft** (logs + records an error; `/api/lookup` just returns empty), so nothing is broken, but the ZIP lookup is inert until the source is corrected. Next step: list the live directory from the VM (`curl …/rel2020/zcta520/`) to find the real filename, or switch to a verified public ZIP→CD source. ZCTA≈ZIP is the intended lookup approximation (avoids the auth-gated HUD crosswalk).
