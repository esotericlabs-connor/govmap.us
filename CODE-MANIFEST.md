...
- **Deploy API budget.** `refresh_all` now also fires ~537 Congress.gov calls (`member_sponsored`) and ~537 OpenFEC calls (`fec_finance`) per run. Both back off on 429, and both also run on their own daily schedule, but several redeploys within one hour can brush the per-key hourly limits (Congress.gov ~5000/hr, api.data.gov ~1000/hr) — non-fatal (records an error, retries next run).
