...
- **Campaign finance needs `FEC_API_KEY`.** `fec_finance.py` (OpenFEC candidate totals) is fail-soft: without a free DATA.gov key in the VM `.env` it records a non-fatal error and the member page simply shows no finance section. Add the key to activate it. v1 is topline totals only — **named top donors + industries** (Schedule A itemization or OpenSecrets, which needs its own key/attribution) is the next finance step.
